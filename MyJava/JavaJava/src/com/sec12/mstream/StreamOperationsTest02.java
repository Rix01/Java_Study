...
public class StreamOperationsTest02 {
	public static void main(String[] args) {
		...
		System.out.println();
		List<String> result = Arrays.stream(str.split("\s+")).filter(word -> word.length() >= 5)
				.map(String::toUpperCase).collect(Collectors.toCollection(ArrayList::new));
		System.out.println(result);

		System.out.println();
		result = Arrays.stream(str.split("\s+")).filter(word -> word.length() >= 5).map(String::toUpperCase)
